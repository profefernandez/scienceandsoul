...
**How to apply:** Do not give sections opaque backgrounds; use `color-mix(... , transparent)` tints if a band effect is needed. Dark mode uses a dimmed overlay on the same asset (see `[data-theme="dark"] .washbg`). Never use `background-attachment: fixed` (broken on iOS) — the fixed-position `.washbg` div is the mobile-safe mechanism. The botanical garland in the contact section is decor, not background — leave it in place, don't replicate it.
